...
Explorez les économies réalisées sur les hôtes détectés : Workload Factory se connecte à vos serveurs Microsoft SQL existants et récupère les détails dans la calculatrice pour une comparaison automatique.  Vous devrez accorder des autorisations de _lecture seule_ ou de _lecture/écriture_ pour utiliser cette option de calculatrice. Vous pouvez modifier le cas d'utilisation, mais tous les autres détails sont automatiquement déterminés dans le calcul.
...
En mode _lecture/écriture_, vous pouvez déployer le nouveau serveur Microsoft SQL sur AWS EC2 à l'aide de FSx for ONTAP directement depuis Workload Factory.  Vous pouvez également copier le contenu de la fenêtre Codebox et déployer la configuration recommandée à l’aide de l’une des méthodes Codebox.
