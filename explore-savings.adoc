...
. Sélectionnez le menu image:workloads-menu-icon.png["L'icône du menu hamburger permet d'accéder aux charges de travail telles que le stockage, l'analyse de données d'entreprise (EDA), l'intelligence artificielle (IA), les bases de données, VMware et l'administration."] puis sélectionnez *Bases de données*.
. Dans le menu Bases de données, sélectionnez *Explorer les économies*.
. Dans Explore savings, sélectionnez l’onglet *SQL Server sur site*.
. Dans l'onglet sur site de SQL Server, téléchargez le script pour évaluer vos environnements SQL Server sur site.
+
.. Téléchargez le script d'évaluation. Le script est un outil de collecte de données basé sur PowerShell. Il recueille puis importe les données de configuration et de performance de SQL Server dans Workload Factory. L'assistant de migration analyse les données et planifie le déploiement de FSx for ONTAP pour votre environnement SQL Server.
+
